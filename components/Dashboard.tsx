...
const Dashboard: React.FC<DashboardProps> = ({ orders, products, leads, currentUser, moderators = [] }) => {
  ...
  const metrics = useMemo(() => {
    ...
    const yesterdayRev = yesterdayOrders.reduce((sum, o) => sum + o.totalAmount, 0);
  ...
  const weeklyData = useMemo(() => {
    ...
    return last7Days.map(date => ({
      ...
      revenue: orders.filter(o => o.createdAt.startsWith(date)).reduce((sum, o) => sum + o.totalAmount, 0)
    }));
  }, [orders]);
  ...
  return (
    <div className="space-y-10 pb-20 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="bg-slate-950 p-5 rounded-[2rem] shadow-2xl flex items-center gap-6 border border-white/10">
           <div className="text-right">
             <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">System Date</p>
             <p className="text-xs font-black text-white">{new Date(todayStr).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
           </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Network Revenue', val: `৳${metrics.totalRev.toLocaleString()}`, sub: 'Lifetime Volume', color: 'text-slate-900', icon: '💎', trend: '+12% vs last month' },
          { label: "Today's Intake", val: `${metrics.todayCount} Ops`, sub: `৳${metrics.todayRev.toLocaleString()} Generated`, color: 'text-indigo-600', icon: '🚀', trend: metrics.growth >= 0 ? `+${metrics.growth}% vs yesterday` : `${metrics.growth}% vs yesterday` },
         ...
         <div className="lg:col-span-2 bg-white p-10 rounded-[4rem] border border-slate-100 shadow-sm">
            <div className="flex justify-between items-center mb-10">
               <div>
                 ...
                 <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Transaction flow last 7 days</p>
               </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="bg-white p-10 rounded-[4rem] border border-slate-100 shadow-sm relative overflow-hidden">
           ...
           <h3 className="text-lg font-black text-slate-800 mb-10 border-b border-slate-50 pb-4 italic">Logistics Radar</h3>
           <div className="h-[250px]">
              <ResponsiveContainer width="100%" height="100%">
                 <PieChart>
                    <Pie data={statusData} innerRadius={65} outerRadius={90} paddingAngle={8} dataKey="value" stroke="none">
           ...
           <div className="grid grid-cols-2 gap-3 mt-6">
              {statusData.slice(0, 4).map((d, i) => (
                <div key={i} className="flex items-center gap-2 bg-slate-50 p-3 rounded-2xl border border-slate-100">
                   ...
                   <span className="text-[9px] font-black text-slate-500 uppercase truncate">{d.name}: {d.value}</span>
                </div>
              ))}
           </div>
        </div>

        <div className="lg:col-span-2 bg-white p-10 rounded-[4rem] border border-slate-100 shadow-sm relative overflow-hidden">
           <div className="absolute top-0 right-0 w-48 h-48 bg-rose-50 rounded-full translate-x-1/2 -translate-y-1/2 opacity-50"></div>
           <div className="flex justify-between items-center mb-10">
              <div>
                <h3 className="text-lg font-black text-slate-800 italic">Inventory Criticality Radar</h3>
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Stock monitoring & alerts</p>
              </div>
              <span className="px-5 py-2 bg-rose-50 text-rose-500 text-[9px] font-black rounded-full uppercase tracking-widest animate-pulse border border-rose-100">Action Required</span>
           </div>
           
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {lowStock.length > 0 ? lowStock.map(p => (
                <div key={p.id} className="p-8 bg-slate-50 rounded-[3rem] border border-slate-100 hover:border-rose-200 transition-all group">
                   <p className="text-xs font-black text-slate-800 leading-tight mb-6 group-hover:text-rose-600 transition-colors uppercase tracking-tight">{p.name}</p>
                   <div className="flex justify-between items-end">
                      <div>
                        <p className="text-3xl font-black text-slate-900 leading-none">{p.stock}</p>
                        <p className="text-[8px] font-black text-slate-400 uppercase mt-2 tracking-widest">In Storage</p>
                      </div>
                      <div className="w-12 h-12 bg-white rounded-2xl shadow-sm flex items-center justify-center grayscale group-hover:grayscale-0 transition-all text-xl">🛒</div>
                   </div>
                </div>
              )) : (
                <div className="col-span-full py-20 flex flex-col items-center justify-center opacity-30 italic">
                   <span className="text-6xl mb-6">🛡️</span>
                   <p className="text-xs font-black uppercase tracking-[0.4em]">All Inventory Systems Nominal</p>
                </div>
              )}
           </div>
        </div>
