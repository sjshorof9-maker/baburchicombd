...
interface ModeratorManagerProps {
  ...
  leads: Lead[];
  orders: Order[];
  onAddModerator: (moderator: any) => Promise<boolean | undefined>;
...
const ModeratorManager: React.FC<ModeratorManagerProps> = ({ moderators, leads, orders, onAddModerator, onDeleteModerator, onToggleStatus }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // CRITICAL: We pass an object WITHOUT the 'id' field.
    // The Supabase table uses 'GENERATED BY DEFAULT AS IDENTITY' which handles the auto-increment.
    const newModerator = {
      name: name.trim(),
  ...
  return (
    <div className="space-y-10 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h2 className="text-5xl font-black text-slate-900 tracking-tighter italic uppercase leading-none">Team Intelligence</h2>
        ...
        <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-100 animate-in slide-in-from-top-4 duration-500 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-4 mb-8">
              <div className="w-12 h-12 bg-indigo-600 text-white rounded-2xl flex items-center justify-center text-2xl shadow-lg font-black italic">?</div>
              <div>
                <h3 className="text-xl font-black text-slate-800 tracking-tight uppercase italic leading-none">Identity Enrollment</h3>
            ...
            <div key={modIdStr} className={`group relative bg-white p-8 rounded-[3rem] border transition-all duration-500 hover:shadow-2xl hover:shadow-indigo-500/10 hover:-translate-y-1 ${!isActive ? 'border-rose-100 bg-rose-50/10' : 'border-slate-100'} ${isProcessing ? 'opacity-40' : ''}`}>
              ...
              <div className="flex items-center gap-6 mb-8">
                ...
                <div className="overflow-hidden">
                  <h3 className="text-xl font-black text-slate-900 leading-none truncate uppercase italic tracking-tighter">{mod.name}</h3>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2 truncate font-mono">{mod.email}</p>
                </div>
              </div>
